...
来源准备 → 采集+提取 / 文生图 / 图生图 → 可选抠图 → 可选侵权检测 → PS 套版 → 标题生成。
当前完整任务页按三段组织：印花来源 / 是否抠图 / 侵权检测 + 套版 + 标题生成。
来源区当前只暴露采集+提取、文生图、图生图，`existing_prints` 只保留在底层兼容入口里。
首版不包含上架，不是自由流程编辑器。
...
- **完整任务** 复用已有模块 runner，不模拟 UI 点击；当前 UI 只暴露采集+提取、文生图、图生图三类来源，`existing_prints` 仅作底层兼容。
