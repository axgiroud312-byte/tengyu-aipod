...
| 8 | 上架 | ✅ 已完成 | Temu + Shein |
| 完整任务 | ✅ 最初版 | 填印花货号，来源 → 可选抠图 → 可选检测 → 等待套版 → PS → 标题 |
...
| **关键边界** | 不包含上架；必须填印花货号；只支持取消，不支持暂停/恢复；因含 PS，macOS 不可启动 |
| **ADR** | `docs/adr/0012-complete-task-initial-fixed-pipeline.md` |

完整任务里的印花货号就是等待套版图片名和套版后货号文件夹名。比如填 `GzG0023`，最终单张印花会先复制成 `02-印花工作区/等待套版/{runId}/GzG0023.png`，PS 输出就是 `04-上架工作区/{模板批次}/GzG0023/`。
