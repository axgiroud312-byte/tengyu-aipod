...
export async function replaceShopName(
  ...
  return runWithActionError(page, 'replaceShopName', async () => {
    ...
    const hit = await locateFirst(page, TEMU_POP_SELECTORS.shop_name_control, 'replaceShopName')
...
export async function fillTitle(page: Page, title: string): Promise<TemuPopDraftPageState> {
  return fillTextField(page, 'fillTitle', title, TEMU_POP_SELECTORS.title_input, 'title_field')
}

export async function fillEnglishTitle(page: Page, title: string): Promise<TemuPopDraftPageState> {
  return fillTextField(
    page,
    'fillEnglishTitle',
    title,
    TEMU_POP_SELECTORS.english_title_input,
    'english_title_field',
  )
}

export async function fillSku(page: Page, sku: string): Promise<TemuPopDraftPageState> {
  return fillTextField(page, 'fillSku', sku, TEMU_POP_SELECTORS.sku_input, 'sku_field')
}
...
export async function uploadMaterialImages(
  ...
  return uploadImagesViaCarouselTrigger(page, files, 'uploadMaterialImages', options)
}
...
export async function uploadCarouselImages(
  ...
  return uploadImagesViaCarouselTrigger(page, files, 'uploadCarouselImages', options)
}

async function uploadImagesViaCarouselTrigger(
  page: Page,
  files: readonly string[],
  action: Extract<TemuPopActionName, 'uploadMaterialImages' | 'uploadCarouselImages'>,
  options: MutationOptions,
): Promise<UploadActionResult> {
  ...
  return runWithActionError(page, action, async () => {
    ...
    if (!options.allowMutation) {
      throw actionFailure({
        ...
        selector: before.carousel_images.selector,
        beforeState: before,
      })
    }
    if (!before.carousel_images.found) {
      throw actionFailure({
        action,
    ...
    const trigger = await locateFirst(page, TEMU_POP_SELECTORS.carousel_upload_button, action)
    await setFilesThroughDropdown(page, trigger.locator, '本地图片', existingFiles, action)
    const after = await waitForState(
      page,
      (state) =>
        state.carousel_images.count > before.carousel_images.count || state.failure_toast.found,
      DEFAULT_ACTION_TIMEOUT_MS,
    )
    if (after.failure_toast.found) {
      throw actionFailure({
        action,
        code: 'UPLOAD_COUNT_MISMATCH',
        message: after.failure_toast.message ?? '轮播图上传后页面出现失败提示',
    ...
    return {
      action: 'uploaded',
      files: existingFiles,
      beforeCount: before.carousel_images.count,
      afterCount: after.carousel_images.count,
      selector: trigger.selector,
    }
  })
}
  ...
  return runWithActionError(page, 'uploadVideo', async () => {
    ...
    if (!before.video_section.upload_button_found || !before.video_section.upload_button_enabled) {
      throw actionFailure({
        action: 'uploadVideo',
        code: 'SELECTOR_NOT_FOUND',
        message: '未找到可用的产品视频上传入口',
        beforeState: before,
        selector: before.video_section.selector,
      })
    }
    const trigger = await locateFirst(page, TEMU_POP_SELECTORS.video_upload_button, 'uploadVideo')
    await setFilesThroughDropdown(page, trigger.locator, '本地上传', existingFiles, 'uploadVideo')
    const after = await waitForState(
      page,
      (state) =>
        state.video_section.current_video_count > before.video_section.current_video_count ||
    ...
    if (after.failure_toast.found) {
      throw actionFailure({
        action: 'uploadVideo',
        code: 'UPLOAD_COUNT_MISMATCH',
        message: after.failure_toast.message ?? '视频上传后页面出现失败提示',
        selector: trigger.selector,
        beforeState: before,
        afterState: after,
      })
    }
    if (after.video_section.current_video_count <= before.video_section.current_video_count) {
      throw actionFailure({
        action: 'uploadVideo',
        code: 'UPLOAD_COUNT_MISMATCH',
        message: '视频上传后数量未增加',
    ...
    return {
      action: 'uploaded',
      files: existingFiles,
      beforeCount: before.video_section.current_video_count,
      afterCount: after.video_section.current_video_count,
      selector: trigger.selector,
    }
  })
}
...
export async function generateSkuCode(
  ...
  return runWithActionError(page, 'generateSkuCode', async () => {
    ...
    const trigger = await locateFirst(
      page,
      TEMU_POP_SELECTORS.one_click_sku_button,
      'generateSkuCode',
    )
    await trigger.locator.click({ timeout: DEFAULT_ACTION_TIMEOUT_MS })
    const modal = page.locator('.ant-modal-content').filter({ hasText: 'SKU' }).last()
    await modal.waitFor({ state: 'visible', timeout: DEFAULT_ACTION_TIMEOUT_MS })
    const input = modal
      .locator('input[placeholder*="编号"], input[placeholder*="前缀"], input:not([type="file"])')
      .first()
    await input.fill(prefix)
    await modal
      .getByRole('button', { name: /设置|确定/ })
      .click({ timeout: DEFAULT_ACTION_TIMEOUT_MS })
    await modal.waitFor({ state: 'hidden', timeout: DEFAULT_ACTION_TIMEOUT_MS })
...
async function fillTextField(
  page: Page,
  action: Extract<TemuPopActionName, 'fillTitle' | 'fillEnglishTitle' | 'fillSku'>,
  value: string,
  selectors: readonly ListingSelector[],
  stateKey: 'title_field' | 'english_title_field' | 'sku_field',
): Promise<TemuPopDraftPageState> {
...
async function setFilesThroughDropdown(
  page: Page,
  trigger: Locator,
  localMenuText: '本地图片' | '本地上传',
  files: string[],
  action: Extract<
    TemuPopActionName,
...
function normalizeRequiredValue(value: string, action: TemuPopActionName): string {
