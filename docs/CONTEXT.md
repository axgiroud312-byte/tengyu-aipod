...
**客户 / Customer**：
购买方或运营主体；后台记录其姓名、手机号、微信备注、邮箱和销售备注。当前 v1 只把客户作为管理记录，不作为客户端授权门禁。
_Avoid_: 用户（"用户"指代实际操作软件的人）、授权账户、设备账户

### 服务器与配置

**云端 / Server**：
腾域的中央服务器（Next.js + Postgres）。**只管理客户记录、Skill 系统提示词、公告和版本，不接触图片、不代理生图、不存用户 API Key 或业务数据**。
_Avoid_: 后端、Cloud
...
**Admin 后台 / Admin**：
管理员浏览器界面（Next.js 中的 /admin），用于管理客户记录、Skill 系统提示词、公告、版本。
...
- 一个 **客户** 是后台管理记录，可被标记为正常或封号。
- 客户记录不影响客户端启动和业务模块运行。
- 后续如果要重新引入登录、授权或席位，必须新增 ADR，不复用旧本地授权码或硬件绑定方案。
...
> **产品经理**：客户管理会不会卡住客户端使用？
> **架构师**：不会。当前 v1 没有客户端授权拦截，客户只是后台记录。客户端能否工作取决于本地工作区、API Key、外部服务和模块配置是否正确。
...
- "用户"曾混指购买方和软件使用者；明确购买方叫**客户 / Customer**，泛指操作软件的人才叫用户。
