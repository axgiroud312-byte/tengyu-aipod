...
- `02-印花工作区` 下固定分 4 个能力目录：`文生图` / `图生图` / `提取` / `抠图`。每次生图操作在能力目录下创建 `{任务名}` 结果文件夹；默认“能力-时间”，前端可自定义。
- `03-检测工作区/{任务名}` 下固定分 `通过` / `复查` / `失败` 三个分类目录。
- 业务工作区目录里**只放业务图片**；元数据、血缘、状态和套版候选清单全部在 `.workbench/workbench.db`。
- **04-上架工作区** 下按 **模板批次** 分一级目录，每个批次内按 **货号** 分二级目录，含成品图 + `titles.xlsx`。
...
> **架构师**：系统先创建一个**轻量任务**（绑定临时货号），编排到 **comfyui-chenyu provider** 的 adapter。adapter 刷新设置页选定的**默认云机**状态；如果它不是运行中，直接提示用户去设置页开机。运行中时，客户端拉取最新 ComfyUI **工作流包**（图生图类别），按 input_slots 注入用户的源印花，提交到默认云机的 ComfyUI HTTP API。完成的印花产物落到 `02-印花工作区/图生图/{任务名}` 下，**印花 ID** 写数据库。
