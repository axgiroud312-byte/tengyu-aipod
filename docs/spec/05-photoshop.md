...
8. 任务开始时对每个模板输出路径画像日志（`template_path_profile`）：有切片标 `fast_path_ok`，无切片标 `slow_export`。
9. 模板级批处理默认每成功完成约 25 个货号组，清理一次 Photoshop 历史缓存（`purge_histories`）；原生切片路径会先恢复模板快照再 purge 并重新建立 history 快照。不改变成品图内容。
