...
- 同一完整任务内多个 ComfyUI 提取 / 文生图 / 图生图 / 抠图步骤选择同一台运行云机时，按固定步骤顺序使用，不并发抢占同一云机。
...
- 启用 PS 套版时，完整任务会把可套版印花复制到 `02-印花工作区/等待套版/{runId}/`。
  图片命名为 `{清洗后的印花货号}{清洗后的分隔符}{四位序号}.{ext}`，序号从 `0001` 开始，仅在本次完整任务内递增，例如 `gyxkj-0001.png`、`gyxkj-0002.png`。
...
- 完整任务内部多个 ComfyUI 步骤选择同一晨羽实例时，按完整任务步骤顺序串行执行。
