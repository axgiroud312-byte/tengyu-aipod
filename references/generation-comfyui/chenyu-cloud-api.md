...
> 抓取自官方文档 2026-05-23，作为 comfyui 生图模块对接的工程参考。
> 官方文档持续更新，**实施前重新核对**。
...
**对腾域的意义**：列出含 ComfyUI 的 Pod，让用户选 → 取 `uuid` 给 `create_by_pod`。
...
**永久删除所有数据**。腾域 v1 **不调用此接口**，避免误删用户数据。
如需"换型号"应当通过 shutdown + startup(gpu_uuid 不同) 实现。
...
创建实例时:
  ↓
立即调 shutdown_timer 设定 N 小时后强制关机（N 默认 1）
  ↓
任务跑完时:
  ↓
弹气泡："任务完成。继续保留 1 小时 / 立即关机 / 修改自动关机时长"
  ↓
用户选"立即关机" → 调 /instance/shutdown
用户选"修改时长" → 调 /instance/shutdown_timer 改 enable=true 新时间戳
用户不操作 → 由定时关机兜底（不会无限计费）
```
...
工作流 JSON 由腾域**自己的云服务器派发**（符合 ADR 思路：服务器不代理生图）。

未来如果晨羽工作流 API 转正且稳定，可以考虑做适配层切换。
...
```
1. /balance/info        验证 Key + 显示余额
2. /pod/list            列出含 ComfyUI 的 Pod（按关键字筛选 "ComfyUI" / "AI 绘图"）
3. /gpu/list            列出 GPU（按价格排序，状态=1）
4. 用户选 Pod + GPU + 自动关机时长 → 显示预估费用
5. /instance/create_by_pod
6. /instance/shutdown_timer  立即设定时关机（兜底）
7. 轮询 /instance/info（每 5 秒）直到 status=2
8. 从 server_map 取 ComfyUI URL → 客户端记录为"当前活跃实例"
```

### 8.3 ComfyUI 调用流程

```
（实例已就绪，ComfyUI URL 已拿到）

1. POST {comfyui_url}/upload/image    上传素材图
2. 加载本地缓存的工作流 JSON          云服务器派发，按版本本地缓存
3. 把图片名注入到 workflow 的 input 节点
4. POST {comfyui_url}/prompt          提交工作流，拿 prompt_id
5. 轮询 GET {comfyui_url}/history/{prompt_id}  每 2 秒
6. 完成 → 从 history.outputs 找产物文件名
7. GET {comfyui_url}/view?filename=xxx  下载产物
8. 落到 02-生图/comfyui/01-提取/{印花ID}/xxx.png
9. 数据库登记血缘（来源、工作流ID、版本、provider="comfyui"）
```
...
| 实例 status=21/22 | 实例关了 → 提示用户重新开机 |
| ComfyUI 连接超时 | 实例还没起好或网络问题 → 提示重试 |
...
- 模块面板顶部：余额（每 60 秒 `/balance/info`）+ 本次开机已运行 X 分钟 × Pod.price.hour + GPU.price.hour（估算）
- 余额 < 10 元 → 红色警告
- 余额 < 1 元 → 阻止启动新任务
...
- [ ] `balance` 和 `card_balance` 是否都能用于实例计费
