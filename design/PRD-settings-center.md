...
14. As a 操作员, I want 运行完成后导轨定格为战报(每阶段产出数+耗时,总耗时), so that 我一眼看到这次运行的成果汇总。
...
- 本 PRD 只覆盖重构第一批:视觉收敛(P1)+ 流水线导轨与运行剧场(P2)。预设系统、阶段配置抽屉化、设置页/Onboarding 重组、演示 mock 模式属于第二批,另立 PRD。
...
- 集中处理的映射规则至少包括:steps/items 到各阶段计数的归并;skipped 阶段的表达;检测 block 归入 blocked;"全拦截零产出=正常完成+警告"的状态语义;取消软停态;中断(interrupted)态;完成态战报数据(每阶段产出+耗时)。
...
- **单元测试(vitest,新增)**:
  - progress-mapper:覆盖流式多阶段同时活跃、skipped、blocked 归类、单项失败累计、全拦截零产出=正常完成+警告、取消软停、中断、完成战报聚合等映射规则。
...
- 设置页三 tab 重组、Onboarding 三步重排、"测试连接"死按钮修复——第二批 PRD。
...
- 第二批 PRD(预设/抽屉/设置重组/演示模式)待本批验收后另行提交,届时预设持久化先走 localStorage、写明升级 SQLite 的路径。
