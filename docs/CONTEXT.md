...
**模板批次 / Batch**：
完整任务和历史兼容布局中，PS 套版输出域 `04-上架工作区/` 下对应一个 PSD 模板输出的一级目录。独立 PS 套版默认改用 **单次套版批次**，不再把 PSD 模板名作为业务一级目录。
_Avoid_: 批次、Output Folder

**单次套版批次 / Standalone Mockup Batch**：
独立 PS 套版默认在 `04-上架工作区/套版-{时间戳}/` 下创建的一级目录，其下按 **货号** 分文件夹，成品图文件名带模板名前缀，例如 `{货号}/{模板名}-01.jpg`。
_Avoid_: 模板批次（独立套版默认不按模板建一级目录）
...
- 独立 PS 套版默认一个 **套版输入印花** 对应 **单次套版批次** 下的一个同名 **货号** 文件夹，多 PSD 模板输出用模板名前缀区分文件。
...
- 独立 PS 套版默认在 **04-上架工作区** 下按 **单次套版批次** 分一级目录，每个批次内按 **货号** 分二级目录，成品图直接在货号文件夹内。
