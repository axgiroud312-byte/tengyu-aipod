...
- 完整任务调用检测时，检测输出目录必须使用完整任务的任务名（用户未填时使用自动生成的完整任务名），形如 `03-检测工作区/{任务名}/{无风险|疑似|高风险}/`；不能按单张图片、流式子任务或 PS 模板名拆分目录。
- 完整任务检测和单独检测允许同时运行，各自使用自己的并发设置；用户需要自行关注百炼请求峰值和费用。
- 完整任务按本次 `concurrency` 合并已经就绪的印花（不额外等待尚未到达的图片），一次调用检测批次；同一批复用一个预处理 Worker Pool，并由检测服务在批内并发执行。Worker 异常退出时当前图片明确失败，池自动补位，后续图片可继续。
