...
完整任务最初版把标题生成作为最后一个固定 step：

- 输入目录来自 PS 套版输出的一个或多个模板批次目录。
- 完整任务中的货号文件夹名来自 PS 使用的等待套版印花文件名；该文件名由完整任务顶部的 **印花货号** 生成。
- 每个模板批次各自写入 `{标题文件名}.xlsx`，默认仍是 `标题.xlsx`。
- 已有标题策略沿用标题模块配置，首版默认跳过已有标题。
- 标题生成完成后完整任务结束；上架仍由上架模块单独读取 `04-上架工作区`。
