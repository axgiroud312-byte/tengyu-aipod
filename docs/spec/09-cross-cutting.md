...
- 完整任务进入 `completed` / `failed` / `cancelled` / `interrupted` 后，释放其占用的采集文件夹读锁、ComfyUI 实例占用和 Photoshop 互斥占用。
...
- 用户取消 → 当前完整任务最终标记 `cancelled`；不再接新活，已在跑的单张尽量跑完。
- 可选步骤关闭时记录为 `skipped`。
...
│   - 创建云机：默认收起，只允许杭州慎思comfyui镜像，选择版本、GPU 和自定义名称
│   - 实例管理：列出当前 API Key 下全部实例，支持改名、开机、关机、设为默认云机
│   - 高级设置：刷新目标 POD / 版本（只读）、重启、销毁
│
├─ 性能
│   - 全局并发：[3] (1-10)
│   - 预处理并发：自动 / [手动: 4]
