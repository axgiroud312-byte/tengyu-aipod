...
| 8 | 上架 | ✅ 已完成 | Temu + Shein |
| 完整任务 | ✅ 最初版 | 任务起点 → 可选抠图 → 可选检测 → 可选等待套版 → 可选 PS → 可选标题；不含上架 |
...
sequenceDiagram
    ...
    桌面->>桌面: 可选，复制为等待套版/GzG0023-0001.png
...
| **晨羽设置** | `SettingsPage.tsx` ← API Key 连接状态、杭州慎思 POD 版本选择、实例命名、全部实例开关机、设为默认云机 |
...
| **关键边界** | 不包含上架；任务起点支持生成新印花或已有印花来源；启用 PS 时才必须填印花货号并要求 Windows；只支持取消，不支持暂停/恢复；关闭 PS 后 macOS 可运行前置步骤 |
| **ADR** | `docs/adr/0012-complete-task-initial-fixed-pipeline.md` + `docs/adr/0013-complete-task-explicit-stage-toggles.md` |

完整任务的任务起点支持采集+提取、Grsai / ComfyUI 文生图、Grsai / ComfyUI 图生图，以及已有印花来源。已有印花来源只能选择 `02-印花工作区` 下的具体印花图片文件夹，可从抠图、侵权检测或 PS 套版开始。启用 PS 套版时，完整任务里的印花货号会生成等待套版图片名，并成为套版后货号文件夹名。比如填 `GzG0023`，最终单张印花会先复制成 `02-印花工作区/等待套版/{runId}/GzG0023-0001.png`，PS 输出就是 `04-上架工作区/{模板批次}/GzG0023-0001/`。
...
flowchart LR
    ...
    C -->|通过| D[等待套版<br/>GzG0023-0001.png]
    D --> E[PS 套版<br/>Windows Photoshop COM]
    E --> F[04-上架工作区/模板批次/GzG0023-0001/]
    F --> G[标题生成<br/>百炼]
