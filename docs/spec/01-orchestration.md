...
完整任务顶部要求填写 **印花货号**，它是本次最终印花文件名和套版后货号文件夹名的共同前缀。
...
- 印花货号使用货号格式：`/^[A-Za-z0-9_-]{1,60}$/`，禁空格、中文和 Windows 保留名。
- 最终进入 PS 前，完整任务会把可套版印花复制到 `02-印花工作区/等待套版/{runId}/`。
  单张印花命名为 `{印花货号}.{ext}`；多张印花命名为 `{印花货号}-01.{ext}`、`{印花货号}-02.{ext}`。
- 局部印花默认启用抠图，满印默认关闭抠图。
- 侵权检测可选；完整任务页直接复用检测模块设置面板，默认 `block` 拦截、`pass` 和 `review` 放行，也可切到“仅无风险通过”让 `review` 一起拦截。
- 完整任务页右侧显示来源图、提示词和运行产物预览，进度事件带最近一批生图/提取/抠图产物。
