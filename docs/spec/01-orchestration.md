...
- 标题逐货号生成：每个货号一套版完成就立刻生成其标题，并累加写进该模板批次的 `标题.xlsx`；xlsx 被 Excel 占用写不进去时，先把该货号标题结果暂存，待可写时补写，不阻塞流水线、不报错。
...
- 首版不支持暂停 / 恢复 / 断点续跑。软件中途关闭后不自动续跑：该 run 标"已中断"，已生成的印花、套好的版、写好的标题都保留在硬盘和数据库，用户想继续就手动从"已有印花来源"起新 run。
