...
**腾域 aipod** = 跨境电商运营桌面工作台。采集 → 生图 → 检测 → PS 套版 → 标题 → 店小秘上架，一站式集成；服务端只派发 Skill / 公告 / 版本，Provider、模型、Workflow 和 API Key 都在客户端本地管理。
...
1. **4 个业务工作区目录下只放业务图片**（`titles.xlsx` 只允许在上架批次目录），元数据全在 SQLite
...
- ❌ 不在 4 个业务工作区里放 `.json` `.jsx` `.csv` 等非图片文件（除 `titles.xlsx`）
