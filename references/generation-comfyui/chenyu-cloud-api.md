...
**对腾域的意义**：用于高级设置里的 POD 自动发现；主创建入口固定服务杭州慎思 POD，只让用户选版本和 GPU。
...
3. 固定 POD UUID + 用户选择的 pod_tag + GPU → /instance/create_by_pod
4. /instance/shutdown_timer  可选设定时关机（兜底）
5. 轮询 /instance/info（每 5 秒）直到 status=2 且解析到 ComfyUI URL
6. 保存为设置页的"默认云机"
