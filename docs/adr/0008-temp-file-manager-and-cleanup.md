# ADR-0008 — 全局 TempFileManager 管理临时文件，5 大类目录只放最终产物
...
如果这些临时产物混入用户的素材总目录（`01-采集` 到 `05-货号成品`），会：
...
├─ 03-检测/         ← 只放图片
├─ 04-待套版印花/    ← 只放图片
├─ 05-货号成品/     ← 只放图片 + titles.xlsx
└─ .workbench/      ← 黑盒，用户不动
    └─ tmp/         ← 临时文件
```

**强约束**：5 大类目录里**绝对不放 .json、.jsx、.csv 等元数据/中间产物文件**。

只有 05-货号成品 下允许 `titles.xlsx`（这是业务文件，给上架程序读）。
...
| 临时文件混在素材总目录 | 实现简单（不用 TempFileManager）| 用户视图脏乱；难以清理 |
...
- 模块必须遵守约束（不能图省事直接写素材目录）

### 强制约束的措施

- Code review 中检查模块代码不直接写 `01-` 到 `05-` 目录的子文件（图片除外）
- 单元测试验证：模块跑完后素材目录新增的非图片文件 = 0
- 文档（CONTEXT.md / spec/00-overview）明确"5 大类目录只放图片"
