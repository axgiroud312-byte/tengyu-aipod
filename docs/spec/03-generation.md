...
完整任务最初版复用生图模块已有 runner，不通过 UI 自动点击：

- `collection` 来源：从采集目录扫描图片后调用提取能力。
- `txt2img` 来源：调用文生图能力直接产生印花。
- `img2img` 来源：调用图生图能力产生印花。
- `existing_prints` 来源：跳过生图，直接扫描已有印花文件夹。
- 抠图是完整任务中的可选 step；局部印花默认开启，满印默认关闭。

完整任务首版不新增生图 provider，不改变 `02-印花工作区` 的输出规则。
