...
完整任务最初版把 PS 套版作为固定 step：

- 输入来自完整任务上游的印花列表；若启用检测，默认接收 `pass` / `review` 印花；用户选择“仅无风险通过”时只接收 `pass`。
- 完整任务调用 PS 前会先生成 `02-印花工作区/等待套版/{runId}/` 图片副本。副本文件名来自完整任务顶部的 **印花货号**：
  单张为 `{印花货号}.{ext}`，多张为 `{印花货号}-01.{ext}`、`{印花货号}-02.{ext}`。
...
- 因为 PS 套版是固定 step，完整任务首版只能在 Windows 启动，macOS 上入口提示不可用。
