...
- 一个 **印花** 在不同 **模板** 下套版后生成多个 **货号**（分散在多个**模板批次**目录里）。完整任务里，
  **印花货号** 先决定等待套版印花文件名，再成为各模板批次里的同名货号文件夹。
- 一个 **套版输入印花** 对应每个 **模板批次** 下的一个同名 **货号** 文件夹。
...
- 业务工作区目录里**只放业务产物**：`01`-`04` 仍只放业务图片，唯一例外是 `04-上架工作区/{模板批次}/` 下的标题 xlsx；`05-视频工作区` 只放 MP4 视频。元数据、血缘、状态和套版候选清单全部在 `.workbench/workbench.db`。
