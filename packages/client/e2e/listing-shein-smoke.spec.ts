...
import { parseDraftPage } from '../src/modules/listing/platforms/dianxiaomi-shein/page-parser'
import { runListingItem } from '../src/modules/listing/platforms/dianxiaomi-shein/workflow'
...
const evidenceDir = resolve(
  process.cwd(),
  '../..',
  '.trellis/tasks/05-23-listing-shein-smoke/evidence',
)
...
test.describe('shein smoke', () => {
  test.skip(!runRealListing, 'REAL_LISTING=1 is required for real Dianxiaomi smoke tests')

  test('runs the Shein v1 template through the workflow smoke baseline', async () => {
    ...
    try {
      ...
      await page.screenshot({
        path: resolve(evidenceDir, 'shein-smoke.png'),
        fullPage: true,
      })
      ...
      await writeFile(
        resolve(evidenceDir, 'shein-smoke-state-report.json'),
...
function createRealItem(args: {
  ...
  return {
    id: `shein-smoke-${args.template.key}`,
    sku: args.before.sku_field.current_value ?? 'SMOKE-SHEIN',
    title: args.before.title_field.current_value ?? 'Smoke Shein',
    platform: 'shein',
