...
来源准备 → 采集+提取 / 文生图 / 图生图 → 可选抠图 → 可选侵权检测 → 可选 PS 套版 → 可选标题生成。
...
**已有印花来源**：
`02-印花工作区` 下用户已准备好的印花图片文件夹，可作为完整任务中途开始的输入。
...
- **完整任务** 图生图入口使用上传参考图并可删除；上传图保存到 `.workbench/pipeline-runs/{runId}/references/`。
