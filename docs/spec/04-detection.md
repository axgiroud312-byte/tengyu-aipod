...
完整任务最初版把侵权检测作为可选 step，但完整任务页不承载完整检测设置面板。
它只读取侵权检测模块已保存的 Skill、模型、阈值和变量配置，并在完整任务页暴露少量运行时选项：
...
- 完整任务页只允许用户调整启用开关、通过要求和是否压缩图片，并展示当前检测模型 / Skill 摘要；完整阈值、变量和 Skill 仍在独立侵权检测模块维护。
