...
设置页中用户选定的一台晨羽 ComfyUI 实例。所有 `comfyui-chenyu` 生图调用只发送到默认云机；如果默认云机未运行，生图模块提示用户先去设置页开机，不自动开机。
...
- 每个生图能力可通过两种本地运行路径实现：comfyui-chenyu 或 grsai。
...
> **架构师**：系统先创建一个**轻量任务**（绑定临时货号），编排到 **comfyui-chenyu provider** 的 adapter。adapter 刷新设置页选定的**默认云机**状态；如果它不是运行中，直接提示用户去设置页开机。运行中时，客户端拉取最新 ComfyUI **工作流包**（图生图类别），按 input_slots 注入用户的源印花，提交到默认云机的 ComfyUI HTTP API。完成的印花产物落到 `02-生图/02-图生图` 下，**印花 ID** 写数据库。
