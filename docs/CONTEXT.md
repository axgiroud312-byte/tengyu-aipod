...
**完整任务**：
跨多个模块按固定顺序执行的任务，以**流式流水线**方式运行（见 ADR-0015）：一张印花生成后立即顺次进入后续步骤，不等整批生成完毕。
任务起点 → 采集+提取 / 文生图 / 图生图 / 已有印花来源 → 可选抠图 → 可选侵权检测 → 可选 PS 套版 → 可选标题生成。
当前完整任务页在同一模块内按两个平级页签组织：**任务设置** / **运行展示**。任务设置页内按三段组织：任务起点 / 是否抠图 / 侵权检测 + 套版 + 标题生成；运行展示页展示当前选中运行的过程与结果。两个页签没有前后依赖，运行失败、取消或中断后仍可返回任务设置。
启用 PS 套版时，完整任务顶部的 **印花货号** 是本次任务最终印花的业务名前缀；进入 PS 前会把最终印花复制到
`02-印花工作区/等待套版/{runId}/`，文件名按 `{清洗后的前缀}{分隔符}{四位序号}.{ext}` 生成，并让套版后的货号文件夹同名。
任务起点支持采集+提取、文生图、图生图和 **已有印花来源**；已有印花来源可从抠图、侵权检测或 PS 套版开始，起点之前的步骤视为跳过。
首版不包含上架，不是自由流程编辑器。
_Avoid_: 大任务、串联任务（"串联"是动词，不做名词）

**任务坞 / Task Dock**：
Workbench 右侧用于查看和切换当前及最近完整任务的导航区域。它只聚合现有完整任务运行记录和公开事件，不会恢复或修改任务，也不是通用任务模型。
_Avoid_: 任务侧栏、任务面板

**流式流水线 / Streaming Pipeline**：
完整任务的执行方式：每张印花一产出就立即进入下游步骤（抠图→检测→套版→标题），各步骤同时流动、互不等整批；执行单位在套版前是印花，套版后扇出为货号。生图整批仍跑完不停，单张出错只踢自己不连累整条。
_Avoid_: 流水线作业（过泛）、流水（歧义）、批处理（是其被取代的反面）
...
**视频 / Video**：
`05-视频工作区` 下由视频生成模块产出的 MP4 业务文件。视频不是印花，不能进入 PS 套版、侵权检测、标题生成或上架模块。
_Avoid_: 动图、视频印花、视频素材（素材指输入时另行说明）

**Artifact**：
任何由模块产生的本地文件（图片、视频、JSX、临时遮罩、xlsx 等）。每个 artifact 在数据库里登记血缘。
_Avoid_: 输出文件、结果
...
**已有印花来源**：
`02-印花工作区` 下用户已准备好的具体印花图片文件夹，可作为完整任务中途开始的输入；不能直接选择 `02-印花工作区` 根目录或 `等待套版` 目录。
_Avoid_: existing_prints、中途来源、任意图片来源

**套版输入印花**：
完整任务进入 PS 套版前实际交给 PS 的印花图片；它的文件名基名必须等于套版后对应的货号文件夹名。
_Avoid_: 原始印花名、PS 临时图

**首帧图 / First Frame Image**：
图生视频的唯一输入图片，决定生成视频第一帧和输出宽高比。首版只允许 1 张本地图片。
_Avoid_: 参考图（参考生视频用语）、封面图（容易误解为视频封面）

**视频参考图 / Video Reference Image**：
参考生视频中用于保持人物、物品、服饰或配件一致的本地图片。首版允许 1-9 张，并在提示词中用 `[Image 1]`、`[Image 2]` 等编号指代。
_Avoid_: 首帧图、参考图（单独出现时容易和 Grsai 图片图生图混淆）
...
**视频生成模块**：
独立模块，用阿里云百炼 HappyHorse 生成 MP4 视频。首版只支持图生视频和参考生视频：图生视频用 1 张 **首帧图**，参考生视频用 1-9 张 **视频参考图**。视频生成模块不属于生图模块，不产出印花，不进入完整任务。
_Avoid_: 生图视频 Tab、视频生图、视频编辑模块（视频编辑首版不做）
...
**HappyHorse**：
阿里云百炼的视频生成模型族。腾域首版通过用户本地百炼 API Key 直连中国内地 DashScope 原生接口，只接入 `happyhorse-*-i2v` 和 `happyhorse-*-r2v`。
_Avoid_: 视频 provider（行文中优先说 HappyHorse 或 阿里云百炼 HappyHorse）
...
**晨羽 POD / Chenyu Pod**：
晨羽应用市场中的云机环境模板。当前设置页创建云机只允许使用标题精确匹配 `杭州慎思comfyui镜像` 的 POD；用户选择该 POD 提供的版本标签、GPU 和自定义云机名称，主进程创建接口再次校验 POD UUID。
_Avoid_: 任意 POD、手动 POD
...
**视频生成运行期日志 / Video Generation Runtime Log**：
视频生成页内存日志，入口和采集、生图、完整任务一致：页面按钮打开命令行式滚动日志弹窗，并自动滚动到最新行。它只用于当场查看图片校验、任务提交、轮询状态、下载保存路径和失败原因。最多保留最近 `1000` 条，应用重启后消失；不包含 API Key 或 base64 图片。
_Avoid_: 诊断日志、审计日志、历史任务记录

**诊断日志 / Diagnostic Log**：
生图、视频生成、侵权检测、标题生成共用的落盘排障 JSONL。写在 `.workbench/logs/diagnostics/{generation|video|detection|title}/{taskIdOrRunId}.jsonl`，默认开启，用于查看本次发送给 LLM / provider 的完整参数、原始返回、轮询/重试次数、解析失败和跳过决策。API Key、authorization、token、secret、password 永远脱敏；base64 / data URL / Buffer 图片内容只记录 mime、字节数、sha256、长度等元信息。默认保留 7 天，总量上限 1GB，启动和每 24 小时自动清理。
...
**模板批次 / Batch**：
完整任务和历史兼容布局中，PS 套版输出域 `04-上架工作区/` 下对应一个 PSD 模板输出的一级目录。独立 PS 套版的业务一级目录是 **单次套版批次**，模板文件夹位于该批次目录下。
_Avoid_: 批次、Output Folder

**单次套版批次 / Standalone Mockup Batch**：
独立 PS 套版默认在 `04-上架工作区/套版-{时间戳}/` 下创建的一级目录，其下先按 **模板**、再按 **货号** 分文件夹，例如 `{模板名}/{货号}/01.jpg`。
同一批次的模板文件夹名必须唯一；同名或清洗后重名模板依次追加 `-2`、`-3`，不能覆盖已有模板目录。
_Avoid_: 模板批次（独立套版的一级目录仍是单次套版批次）
...
**采集文件夹锁 / Collection Folder Lock**：
完整任务读取某个采集任务文件夹期间，采集模块不能写入同一文件夹或其子目录；不同采集任务文件夹可以并行。
_Avoid_: 全局采集锁（不是禁止所有采集）
...
客户端本地维护的 Grsai、百炼、晨羽模型清单、节点、并发、重试和 API Key 绑定。API Key 默认存本地密钥存储，生产环境走 OS keychain；开发环境 safeStorage 不可用时允许 plain: 兜底，配置不上传云端。
...
**工作区 / Workbench Root**：
用户在设置页指定的本地根目录，腾域在其下创建采集、印花、检测、上架、视频 5 个业务工作区和 `.workbench/` 黑盒。
...
- 在完整任务和模板优先布局中，一个 **印花** 在不同 **模板** 下套版后生成多个 **货号**（分散在多个**模板批次**目录里）。完整任务里，
  **印花货号** 先决定等待套版印花文件名，再成为各模板批次里的同名货号文件夹。
- 独立 PS 套版默认一个 **套版输入印花** 对应 **单次套版批次** 下每个模板文件夹中的同名 **货号** 文件夹。
- 完整任务中，一个 **套版输入印花** 对应每个 **模板批次** 下的一个同名 **货号** 文件夹。
- 一个 **原图** 经"提取"产生一个或多个 **印花**；原图不是印花。
- 一个 **印花** 可用于多个货号（跨模板）。
- 一个 **视频** 是视频生成模块的 MP4 产物；它不是印花，也不对应货号。
...
- **视频生成模块** 是独立入口，使用 HappyHorse 生成 MP4；首版支持图生视频和参考生视频，不支持文生视频、视频编辑或批量队列。
...
- **完整任务** 复用已有模块 runner，不模拟 UI 点击；任务起点支持采集+提取、文生图、图生图和 **已有印花来源**。
- **完整任务** 中每个 ComfyUI 提取 / 文生图 / 图生图 / 抠图步骤都独立选择 **运行云机**；设置页的 **默认云机** 仍只是默认候选。
- **完整任务** 启用 PS 套版时要求填写 **印花货号**。最终印花用户可见文件名使用
  `{清洗后的印花货号}{清洗后的分隔符}{四位序号}.{ext}`，序号从 `0001` 开始，仅在本次完整任务内递增，避免覆盖。
- **完整任务** 从 **已有印花来源** 开始时，可选择从抠图、侵权检测或 PS 套版开始；起点之前的步骤锁定为跳过，后续步骤使用上一阶段的最新产物。
- **完整任务** 的抠图、侵权检测、PS 套版、标题生成由页面开关控制；关闭的步骤记录为 `skipped`。
- **完整任务** 的检测策略默认是 `block` 拦截、`pass` / `review` 放行，并记录 review 数量；用户可切到“仅无风险通过”让 `review` 也拦截。
- **完整任务** 默认带入侵权检测模块已保存的 Skill、模型、阈值等配置；页面允许为本次运行覆盖模型、Skill、通过要求和压缩开关，但不反写单独侵权检测模块默认配置。
- **完整任务** 的任务设置页负责编辑当前会话草稿；运行展示页展示来源图、参考图、印花产物、检测结果和运行期日志，帮助用户在长流程中确认当前效果。两个页签可随时切换。
- **完整任务** 的 Grsai 图生图入口使用上传参考图并可删除；上传图保存到 `.workbench/pipeline-runs/{runId}/references/`。
- **完整任务** 不包含上架；上架仍由上架模块单独读取 `04-上架工作区`。
...
- **工作区** 下固定创建 `01-采集工作区/`、`02-印花工作区/`、`03-检测工作区/`、`04-上架工作区/`、`05-视频工作区/` 和 `.workbench/`。
- `02-印花工作区` 下固定分 4 个能力目录：`文生图` / `图生图` / `提取` / `抠图`。每次生图操作在能力目录下创建 `{任务名}` 结果文件夹；提取后抠图归入 `抠图` 目录，默认“能力-时间”，前端可自定义。
- 完整任务启用 PS 套版时会创建 `02-印花工作区/等待套版/{runId}/`，只放本次最终进入套版的图片副本，文件名来自 **印花货号**、分隔符和四位序号。
- `03-检测工作区/{任务名}` 下固定分 `无风险` / `疑似` / `高风险` 三个分类目录。
- `05-视频工作区/{图生视频|参考生视频}/{任务名}/0001.mp4` 保存视频生成模块产出的视频。任务名由用户填写或自动用时间生成；已存在 `0001.mp4` 时必须报错，不覆盖。
- 业务工作区目录里**只放业务产物**：`01`-`04` 仍只放业务图片，唯一例外是 `04-上架工作区/{上架批次}/` 下的标题 xlsx；`05-视频工作区` 只放 MP4 视频。元数据、血缘、状态和套版候选清单全部在 `.workbench/workbench.db`。
- 独立 PS 套版默认在 **04-上架工作区** 下按 **单次套版批次** 分一级目录，每个批次内先按 **模板**、再按 **货号** 分目录，成品图直接在货号文件夹内。
- 完整任务和历史兼容布局在 **04-上架工作区** 下按 **模板批次** 分一级目录，每个批次内按 **货号** 分二级目录，含成品图 + 标题 xlsx（当前标题模块默认 `标题.xlsx`，上架扫描优先读 `标题.xlsx`，兼容旧 `titles.xlsx`）。
...
> **产品经理**：完整任务或模板优先布局里，用户把同一批印花套到 3 个 PSD 模板上，会产生几个货号？
> **架构师**：3 个模板批次，每个批次内每个印花一个货号。所以 N 个印花 × 3 模板 = 3N 个 listing。每个 **模板批次** 是一个独立的上架批次，对应一份标题 xlsx。

> **产品经理**：完整任务里用户填的"印花货号"到底决定什么？
> **架构师**：它决定等待套版印花的文件名前缀，也决定套版后货号文件夹的名字。比如填 `GzG0023`、分隔符用 `-`，则进入 PS 前会得到 `02-印花工作区/等待套版/{runId}/GzG0023-0001.png`，套版后每个模板批次里都是 `GzG0023-0001/` 文件夹。

> **产品经理**：视频生成出来后能当印花去套版吗？
> **架构师**：不能。视频生成模块产出的是 **视频**，保存到 `05-视频工作区`，只作为 MP4 业务产物管理。它不写 `02-印花工作区`，也不进入 PS 套版、标题生成或上架链路。
...
- "图生视频"不是图片生图模块里的"图生图"。图生视频属于**视频生成模块**，输入是 1 张**首帧图**，输出是 MP4 **视频**。
