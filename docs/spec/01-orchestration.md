...
- 标题逐货号生成：每个货号一套版完成就立刻生成其标题，并累加写进该模板批次的 `标题.xlsx`；xlsx 被 Excel 占用写不进去时，把待写标题持久化到 SQLite `pending_title_writes` 表，该 run 与标题 step 仍正常完成并记录待补写数量。后续同批写入或下一次标题阶段启动时先重试，成功后补登记 `skus` 并删除待补写记录，不阻塞流水线、不把完整任务标为失败（ADR-0015）。
