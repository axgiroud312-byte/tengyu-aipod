...
当前完整任务页按"上方配置、下方结果"组织。配置区包含任务起点 / 是否抠图 / 侵权检测 + 套版 + 标题生成；
结果区按阶段分栏竖排、各栏同时流动展示采集图、参考图、印花产物、侵权检测结果和后续摘要。
...
- 标题逐货号生成：每个货号一套版完成就立刻生成其标题，并累加写进该模板批次的 `标题.xlsx`；xlsx 被 Excel 占用写不进去时，运行中先暂存待补写，最终仍无法写入则该 run 失败，用户关闭 Excel 后手动续跑。
...
- 失败隔离：单张印花或单货号在某步出错只标失败并踢出流水线、记日志，不影响其他印花和整条 run；整条 run 只有"生图起不来"（如未开机、配置错）和"用户取消"才算失败。检测全被拦、零货号产出算正常完成，仅给警告。
