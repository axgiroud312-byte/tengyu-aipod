...
`02-印花工作区/等待套版/{runId}/`，文件名用该前缀，并让套版后的货号文件夹同名。
...
- **完整任务** 启用 PS 套版时要求填写 **印花货号**。单张最终印花使用 `{印花货号}.{ext}`；多张最终印花使用
  `{印花货号}-01.{ext}`、`{印花货号}-02.{ext}`，避免覆盖。
...
- 完整任务启用 PS 套版时会创建 `02-印花工作区/等待套版/{runId}/`，只放本次最终进入套版的图片副本，文件名来自 **印花货号**。
...
> **架构师**：它决定等待套版印花的文件名前缀，也决定套版后货号文件夹的名字。比如填 `GzG0023` 且最终只有一张印花，则进入 PS 前会得到 `02-印花工作区/等待套版/{runId}/GzG0023.png`，套版后每个模板批次里都是 `GzG0023/` 文件夹。
