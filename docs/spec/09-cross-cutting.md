...
- 任一步骤抛错 → 当前完整任务标记 `failed`。
- 用户取消 → 当前完整任务标记 `cancelled`。
- 可选步骤关闭时记录为 `skipped`。
- 侵权检测开启后，`block` 级别图片不进入后续 PS；默认 `pass` 和 `review` 继续，也可配置为只放行 `pass`。
- 如果检测后没有可继续处理的印花，完整任务失败。
- 等待套版副本创建失败时，完整任务失败；该目录是业务图片副本，不走 TempFileManager 清理。

首版没有 `skip` / `pause` 失败策略，也不支持断点恢复。
