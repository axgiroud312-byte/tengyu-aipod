...
文件名自定义只影响用户可见文件名、展示名和 artifact 的 `file_path`，不改变数据库内部印花 ID。每张最终印花仍必须生成全局唯一 `print_id = pri_xxx`，不得用用户可见文件名替代 `print_id`。前缀和分隔符必须清洗 Windows 非法字符 `< > : " / \ | ? *`、控制字符、结尾空格和结尾点；清洗后前缀为空时视为未填写，回退默认命名逻辑。所有 ComfyUI 生图能力再次使用同一任务目录和图片名前缀时，必须在调用外部生图服务前按 Windows 大小写不敏感规则扫描已有最大序号并从下一号继续；已有文件永不覆盖。
...
完整任务调用生图、提取或抠图 runner 时，输出任务目录必须使用完整任务的任务名（用户未填时使用自动生成的完整任务名），
不能使用 PSD 模板名、`runId` 或单张流式子任务名。流式分批调用同一个 runner 时，仍写入同一个任务目录，并按本次完整任务内的产物顺序延续四位序号，避免每张图分散到单独文件夹。
...
- 运行时如果本次显式选择的运行云机就是本地默认云机，且晨羽实例详情暂时无法从 `server_map` / `server_url` 解析 ComfyUI 地址，客户端必须使用本地保存的默认云机 ComfyUI 地址兜底；不能把这个地址用于其他实例。
...
完整任务页中的晨羽实例选择也必须提供刷新入口，并展示所选实例 UUID 和 ComfyUI 地址；没有可选运行云机时提示用户去设置页开机后刷新。
