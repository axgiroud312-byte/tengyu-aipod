...
- ✅ 文档体系完成（PRD / CONTEXT / 10 个 Spec / 15 个 ADR / 4 份 references/）
- ✅ 旧 Trellis 任务资料已清理，当前以代码、docs/spec 和 ADR 为准
- ✅ 切片 0-8 代码层归档完成，v1 全功能（采集 / 检测 / 生图 / PS 套版 / 标题 / 上架）已就位
- ✅ 完整任务最初版已就位：统一流式链按固定顺序运行，来源 → 可选抠图 / 检测 → 可选 PS 套版 → 可选标题生成
...
└─ adr/                          ← 15 个关键架构决策
    ...
    └─ 0015-complete-task-streaming-pipeline.md
...
| **完整任务** | 内置固定顺序跨模块流程，统一按单一路径流式执行；后续步骤按开关执行，不包含上架；启用 PS 时印花货号决定等待套版图片名和 PS 后货号文件夹名 | CONTEXT.md / ADR-0015 |
...
- 需要登录默认测试服务器时，使用 SSH 别名：`ssh test_server`
- 默认允许通过 `ssh test_server` 执行服务器常规操作，例如查看文件、查看日志、查看容器状态、拉取镜像、健康检查和非破坏性排障命令
- 不要直接使用密码登录
- 不要在对话、代码、文档或日志里暴露私钥、服务器密码或其他凭据
- 高风险操作必须先向用户确认，包括：删除文件或数据、清空/删除 Docker volume、删除镜像或容器、重启服务器、停止/重启线上服务、修改生产配置或环境变量、修改防火墙/安全组、执行可能改变数据库结构或数据的命令
...
- ❌ 不在 v1 实现通用可配置编排引擎；当前只允许 ADR-0015 约束下的内置固定顺序完整任务
- ❌ 不在 v1 支持 Temu/Shein 以外的上架平台（详见 spec/07）
- ❌ 不在 4 个业务工作区里放 `.json` `.jsx` `.csv` 等非图片文件（除上架批次目录下的标题 xlsx）
