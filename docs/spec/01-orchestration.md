...
当前 v1 完整任务不支持自动恢复或恢复弹窗，但固定完整任务已支持用户手动续跑：

- 启动时如果发现 `interrupted` 的完整任务，只保留运行记录和已落盘产物，不自动续跑。
- 用户想继续时，可在完整任务记录中手动续跑 `interrupted` / `failed` run；系统按已完成的 source、PS、title item 跳过，继续未完成项。
- 如果等待套版目录、来源图、已落盘产物等续跑必要文件缺失，则拒绝续跑并提示用户从“已有印花来源”或对应模块重新启动新的 run。
...
v1 已实现一个固定顺序完整任务，不是自由流程编辑器，也不包含上架。**执行方式为流式流水线（ADR-0015）**：
每张印花一产出就立即顺次进入后续步骤，各步骤同时流动、互不等整批；生图整批仍跑完不停，单张出错只踢自己不连累整条 run。
当前完整任务页在同一模块内提供两个平级页签：**任务设置** / **运行展示**。任务设置页包含任务起点 / 是否抠图 / 侵权检测 + 套版 + 标题生成；
运行展示页按阶段分栏竖排，各栏同时流动展示采集图、参考图、印花产物、侵权检测结果和后续摘要。两个页签没有前后依赖，运行失败、取消或中断后仍可返回任务设置。
任务起点支持 `collection`、`txt2img`、`img2img` 和 `existing_prints`。`existing_prints` 对应 **已有印花来源**，
只能选择 `02-印花工作区` 下的具体印花图片文件夹，不能直接选择 `02-印花工作区` 根目录或 `等待套版` 目录。
启用 PS 套版时要求填写 **印花货号**，它是本次最终印花用户可见文件名和套版后货号文件夹名的共同前缀。
...
- `existing_prints` 仅代表“使用已有印花作为完整任务起点”，不是自由流程编辑器。配置中记录 `startStep: 'matting' | 'detection' | 'photoshop'`；起点之前的步骤锁定为 `skipped`。
- `existing_prints` 默认 `startStep = 'photoshop'`，并默认开启 PS 套版和标题生成；即使用户改为从抠图或侵权检测开始，标题生成也默认保持开启，但用户可以手动关闭。标题生成仍依赖 PS 套版，缺少标题配置时按现有启动校验提示补齐。
- 从 `matting` 开始时，抠图开启，后续侵权检测、PS 套版、标题生成仍按开关执行；从 `detection` 开始时抠图锁定关闭；从 `photoshop` 开始时抠图和侵权检测锁定关闭。
- 从已有印花来源开始后，后续步骤始终使用上一阶段的最新产物：抠图后用抠图结果，检测后用检测通过结果，直接 PS 时用已有印花来源图片。
- 完整任务里的 ComfyUI 提取和抠图步骤按能力独立选择运行云机；设置页的默认云机只是默认候选，未选择运行中云机时不能启动对应步骤。
- 同一完整任务内多个 ComfyUI 步骤选择同一台运行云机时，可交错往该云机队列提交，由 ComfyUI 内部串行执行（不再按步骤顺序串行）；想跑得更顺可开两台云机分别负责生图和抠图（ADR-0015）。
- 生图函数提供 `onImageComplete` 回调，每张印花算完立即把路径、印花 ID 和 artifact 血缘吐给流水线，流水线立刻送进下一步；生图整批该跑还跑、不停。
- 采集+提取走 ComfyUI 时仍需选择提取 Skill、提取工作流和运行云机；提取 Skill 负责说明从原图里提取什么。
- 文生图统一先走阿里云百炼生成提示词；随后可选 Grsai 文生图或 ComfyUI 文生图工作流，不保留“自己写提示词”模式。
- 图生图支持 Grsai 和 ComfyUI 两条路径：Grsai 路径沿用参考构图 / 参考风格 / 构图+风格参考方式和“生图时带参考图”开关；ComfyUI 路径用图片文件夹、工作流、运行云机和每张生成数量驱动。
- 印花货号作为本次完整任务的用户可见图片名前缀；只有启用 PS 套版时必填。前缀和分隔符会清洗 Windows 非法字符 `< > : " / \ | ? *`、控制字符、结尾空格和结尾点；清洗后为空则视为未填写。
- 启用 PS 套版时，完整任务会把可套版印花复制到 `02-印花工作区/等待套版/{runId}/`。
  图片命名为 `{清洗后的印花货号}{清洗后的分隔符}{四位序号}.{ext}`，序号按印花到达套版的先后顺序发放（先到先得，从 `0001` 起，仅在本次完整任务内递增），例如 `gyxkj-0001.png`、`gyxkj-0002.png`。
- PS 套版只读取等待套版副本；等待套版副本的文件名基名必须等于 PS 输出的货号文件夹名。已有印花来源的原始文件不改名、不移动。
- 自定义命名只影响用户可见文件名、展示名、`file_path` 和等待套版副本名，不改变内部 `print_id = pri_xxx`。
- 局部印花默认启用抠图，满印默认关闭抠图。
...
- 完整任务运行展示页显示阶段结果。采集图和参考图可分页；文生图、图生图、提取和抠图等印花产物按预计产出数量预留槽位，完成一张显示一张，未完成显示加载中，失败槽位隐藏。
...
- 完整任务草稿只在当前软件会话内保留。切换模块或运行展示页后恢复当前配置，应用重启后不恢复；切换运行展示页的历史 run 不得覆盖任务设置草稿。
- 来源图预览可折叠；Grsai 图生图入口使用上传参考图并可删除，上传图保存到 `.workbench/pipeline-runs/{runId}/references/`。
- 标题生成依赖 PS 套版；关闭 PS 时标题自动关闭。
- 标题逐货号生成：每个货号一套版完成就立刻生成其标题，并累加写进该模板批次的 `标题.xlsx`；xlsx 被 Excel 占用写不进去时，把待写标题持久化到 `.workbench/pipeline-runs/{runId}/pending-title-writes/`，该 run 与标题 step 仍正常完成并记录待补写数量。后续同批写入或下一次标题阶段启动时先重试，成功后补登记数据库并清理暂存记录，不阻塞流水线、不把完整任务标为失败（ADR-0015）。
- 启用 PS 套版时完整任务只能在 Windows 启动；未启用 PS 时 macOS 可运行前置步骤。
- 取消为软停：不再接新活（生图停止、未开始的印花/货号不再往下放），已在跑的单张让它跑完，不强制中断外部 API / Photoshop；已完成的结果保留（ADR-0015）。
- 首版不支持暂停、自动恢复弹窗或通用编排引擎级断点续跑。软件中途关闭后不自动续跑：该 run 标"已中断"，已生成的印花、套好的版、写好的标题都保留在硬盘和数据库；用户可手动续跑 `interrupted` / `failed` 的固定完整任务，或从"已有印花来源"起新 run。
- 失败隔离：单张印花或单货号在某步出错只标失败并踢出流水线、记日志，不影响其他印花和整条 run。任一阶段整体无法启动或无法安全继续（例如配置或鉴权错误、云机未就绪、Photoshop 未运行、关键业务文件无法持久化）时，整条 run 标记为 `failed`；用户软停标记为 `cancelled`。检测全被拦、零货号产出算正常完成，仅给警告。

实现上，完整任务复用各模块 runner。流式流水线要求步骤记录支持逐项进度（每张印花/货号的状态），而非只在整批边界更新：
...
- 生图：Grsai 付费模型可与完整任务并行；同一晨羽实例同一时刻只允许一个任务占用。完整任务内部生图与抠图等 ComfyUI 步骤选同一台云机时，可交错往该云机队列提交、由 ComfyUI 内部串行（ADR-0015）。
...
当前 v1 完整任务只支持取消和固定完整任务的手动续跑，不支持暂停、自动恢复弹窗或通用编排引擎级断点续跑；以下内容是 v1.5 通用编排引擎的目标设计，不能当作当前已落地能力。
...
检测 step → 风险通过
  ↓
等待套版准备 → 02-印花工作区/等待套版/{runId}/{prefix}-{0001}.png
  ↓
套版 step → 输出到 04-上架工作区/{batch}/{prefix}-{0001}/
  ↓
