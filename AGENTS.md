...
| **完整任务** | 内置固定跨模块流程，当前跑到标题生成，不包含上架；印花货号决定等待套版图片名和 PS 后货号文件夹名 | CONTEXT.md / ADR-0012 |
...
- ❌ 不在 v1 实现通用可配置编排引擎；当前只允许 ADR-0012 的内置固定完整任务
