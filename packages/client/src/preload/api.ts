...
type PreloadApiContract = Omit<Partial<ClientApi>, 'listing'> &
  ...
  listing: {
    ...
    openPath: (input: { path: string }) =>
      ipcRenderer.invoke('listing:open-path', input) as Promise<
        { ok: true } | { ok: false; error: { code: string; message: string } }
      >,
    run: (input: { config: unknown; items: ListingItem[] }) =>
      ipcRenderer.invoke('listing:run', {
